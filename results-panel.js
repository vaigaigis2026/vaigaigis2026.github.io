...
(function () {
  ...
  // Builds SVG tspan markup for text containing the unicode superscript-two
  // character (km\u00b2 etc.), rendering it as a true raised, smaller "2"
  // rather than relying on a font glyph that may not render as intended.
  function svgTextWithSuperscript(text, fontSize) {
    var idx = String(text).indexOf("\u00b2");
    if (idx === -1) return escapeXml(text);
    var before = text.slice(0, idx);
    var after = text.slice(idx + 1);
    return escapeXml(before) +
      '<tspan baseline-shift="super" font-size="' + (fontSize * 0.7).toFixed(1) + '">2</tspan>' +
      escapeXml(after);
  }
  ...
  function buildChartSVG(dataset, opts) {
    ...
    var axisTitleSize = dataset.axisTitleSize || opts.axisTitleSize || 11;
  ...
  function renderInto(container, resultsData) {
    ...
    datasets.forEach(function (dataset) {
      ...
      var unitLabel = (dataset.unit || "records").replace(/\u00b2/g, "<sup>2</sup>");
